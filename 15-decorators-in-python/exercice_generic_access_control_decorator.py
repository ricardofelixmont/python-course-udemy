...
def access_control(access_level: int):   # primeiro recebe o access_level
    def user_permission(func):
        @functools.wraps(func) # guarda as informações das funções anteriores.
...
@access_control(0)   # Para o decorator passamos como argumento o 'access_level'
def delete_some_file(filename): # Na função passamos o nome do arquivo como 'filename' 
    # perform the deletion operation
    print(f'{filename} is deleted!')

delete_some_file('Nome_do_arquivo')
